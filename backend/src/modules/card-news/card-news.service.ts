...
export class CardNewsService {
  ...
  async generate(episodeId: string): Promise<CardNews> {
    ...
    this.logger.log(`[1/3] 디렉터 분석 시작: episodeId=${episodeId}`);
    const direction = await this.directorService.analyze(episode.script);
    this.logger.log(`[1/3] 디렉터 완료: theme=${direction.theme}, mood=${direction.mood}`);

    this.logger.log('[2/3] 디자인 메이커 HTML 생성 시작');
    const html = await this.designMakerService.generateHtml(direction);
    this.logger.log('[2/3] HTML 생성 완료');

    const outputDir = process.env.CARD_NEWS_OUTPUT_DIR ?? './card-news-images';
    const filename = `${episodeId}-${Date.now()}.png`;
    const outputPath = path.join(outputDir, filename);

    this.logger.log('[3/3] Puppeteer PNG 렌더링 시작');
    const savedPath = await this.rendererService.renderToFile(html, outputPath);
    this.logger.log(`[3/3] 렌더링 완료: ${savedPath}`);
